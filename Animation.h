...
public:
	...
	{
		AnimationSequence& anim = animations[name];
		Matrix scale = Matrix::scaling(anim.interpolate(anim.frames[baseFrame].scales[boneIndex], anim.frames[anim.nextFrame(baseFrame)].scales[boneIndex], interpolationFact));
		Matrix rotation = anim.interpolate(anim.frames[baseFrame].rotations[boneIndex], anim.frames[anim.nextFrame(baseFrame)].rotations[boneIndex], interpolationFact).toMatrix();
		Matrix translation = Matrix::translation(anim.interpolate(anim.frames[baseFrame].positions[boneIndex], anim.frames[anim.nextFrame(baseFrame)].positions[boneIndex], interpolationFact));
		Matrix local = scale * rotation * translation;
		if (skeleton.bones[boneIndex].parentIndex > -1)
		{
			Matrix global = local * matrices[skeleton.bones[boneIndex].parentIndex];
