...
public:
	...
	void wait() {
		if (fence->GetCompletedValue() < (unsigned int)value) {
...
{
public:
	static void add(ID3D12Resource *res, D3D12_RESOURCE_STATES first,
			D3D12_RESOURCE_STATES second, ID3D12GraphicsCommandList4* commandList) {
...
public:
	...
	void init(HWND hwnd, int _width, int _height) {
		...
		for (int i = 0; i < adapters.size(); i++) {
			...
			if (desc.DedicatedVideoMemory > (unsigned long long)maxVideoMemory) {
	...
	void uploadResource(ID3D12Resource* dstResource, const void* data, unsigned int size, 
		...
		HRESULT hr = device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &bufferDesc, D3D12_RESOURCE_STATE_GENERIC_READ, NULL, IID_PPV_ARGS(&uploadBuffer));
