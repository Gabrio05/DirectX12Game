...
public:
	...
	void initConstantBuffers(Core *core, ID3DBlob* shader, std::vector<ConstantBuffer> &buffers)
	{
		...
		for (int i = 0; i < (int)desc.ConstantBuffers; i++)
		{
			...
			for (int n = 0; n < (int)cbDesc.Variables; n++)
			{
...
public:
	...
	void updateTexturePS(Core* core, std::string name, std::string variableName, int heapOffset) {
		shaders[name].updateTexturePS(core, name, heapOffset);
	}
