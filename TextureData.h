...
public:
	...
	void upload(Core* core, const int width, const int height, const void* data) {
...
public:
	...
	void load(Core* core, std::string filename) {
		// Check if filenames is in texstures
		...
		if (channels == 3) {
			...
			tex->upload(core, width, height, texelsWithAlpha);
			delete[] texelsWithAlpha;
		}
		else {
			tex->upload(core, width, height, texels);
		}
		stbi_image_free(texels);
		textures.insert({ filename, tex });
	}
	int find(std::string filename) {  // Returns heapoffset
		return textures[filename]->heapOffset;
	}
};